...
public class GamePanel extends JPanel{
	...
	public void drawText(Graphics g, int x, int y, boolean isBold, Color color, String text) throws IllegalArgumentException{ //a function to draw strings on the JPanel
		Graphics2D g2d = (Graphics2D) g;
		g2d.setRenderingHint(RenderingHints.KEY_ANTIALIASING, RenderingHints.VALUE_ANTIALIAS_ON);
		g2d.setColor(color);
		if(isBold){ 
	...
	public void render(int x, int y, Entity entity, Graphics g){ 
		...
		if(entity.isPlayer()){ //Check if the entity is a player
			...
			//double dir = entity.getDirection(); //get direction //0 NORTH, 1 EAST, 2 SOUTH, 3 WEST; 0.5 NE, 1.5 SE, 2.5 SW, 3.5 NW
			
			//EnumDirection playerdir = dir.getDir();
			//drawText(g2d, x+15, y+45, true, Color.CYAN, playerdir.toString());
			//drawText(g2d, x, y-10, true, Color.GREEN, "Rendered player circle");
		...
		}else if(entity.toString().equals("Bullet")){
